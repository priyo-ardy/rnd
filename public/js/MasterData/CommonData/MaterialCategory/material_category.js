...
buttons.save.addEventListener("click", () => {
    if (validasi()) {
        try {
            loading();
            fetchData(baseurl + '/material_category/save', 'POST', new FormData(dataForm))
                .then((result) => {
...
function editData(token) {
    try {
        loading();
        fetchData(baseurl + '/material_category/edit', 'POST', JSON.stringify({ token: token }))
            .then(result => {
...
buttons.update.addEventListener('click', (e) => {
    if (validasiUpdate) {
        try {
            loading();
            fetchData(baseurl + '/material_category/update', 'POST', new FormData(dataForm))
                .then((result) => {
                    pesanSukses(result.message);
                    hideLoading();
                    clearForm();
...
buttons.export.addEventListener('click', async () => {
    try {
        ...
        const response = await fetch(baseurl + "/material_category/export", {
            method: "GET",
        ...
        a.href = url;
        a.download =
            "material_category_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
